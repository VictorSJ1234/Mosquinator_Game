...
class Enemy2 extends FlameGame {
  ...
  double frameRate = 25; // Adjust this value to control the animation speed

  // New variables for animation
  double timeSinceLastFrame = 0.0;

  Enemy2(this.gameController, double x, double y) {
    health = 3; // set initial health to 2
    damage = 1; // set damage to 1
    speed = gameController.tileSize * 2; // set speed to 2 times the tile size

    // Load spritesheet
    Flame.images.load('enemies/sample_enemy2.png').then((image) {
      spritesheet = image;

      rows = 3; // specify the number of rows in the spritesheet
      columns = 10; // specify the number of columns in the spritesheet

      frameWidth = spritesheet!.width.toDouble() / columns;
      frameHeight = spritesheet!.height.toDouble() / rows;

      double scale = gameController.screenSize.width / frameWidth * 0.35; //size of the enemy
  ...
  void render(Canvas c) {
    if (spritesheet != null) {
      bool flipHorizontally = enemyRect.right > gameController.screenSize.width / 2;

      c.save(); // Save the canvas state

      // Flip the canvas horizontally if the enemy spawns from the right
      if (flipHorizontally) {
